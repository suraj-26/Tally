	...
	<div class="row">
		<div class="col-md-12">
			...
			<select class="form-control" id="reportName" name="reportName">
				<option value="Bills Receivable">Outstanding Receivable</option>
				<option value="Bills Payable">Outstanding Payable</option>

			</select>
			<div class="">
				<button type="button" class="btn btn-primary" onclick="get_trialBalance()">View</button>
				<button type="button" class="btn btn-primary" onclick="download_Excel()">Download Excel</button>
			</div>
		</div>
...
<script>
	...
	function download_Excel() {
		...
		if(company_name == "" || fromDate== "" || toDate==""){
			...
			location.href = "<?= base_url() ?>"+"ExportController/download_ExcelStatementAccount?comp="+btoa(company_name)+"&fromDate="+btoa(fromDate)+"&toDate="+btoa(toDate)+"&reportName="+btoa(reportName);
		}
