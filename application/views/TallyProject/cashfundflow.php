	...
	<div class="row">
		<div class="col-md-12">
			...
			<select class="form-control" id="reportName" name="reportName">
				...
				<option value="Funds Flow">Fund Flow</option>
				<option value="Cash Flow Projection">Cash Flow Projection</option>
			</select>
			<div class="">
...
<script>
	$(document).ready(function () {
		...
		$('#reportName').change(function(){
			$('#downloadexcel')[ ($("option[value='Cash Flow Projection']").is(":checked"))? "hide" : "show" ]();
		});
	});
	...
	function get_report() {
		...
		if(company_name == "" || fromDate== "" || toDate=="" || reportName==""){
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url("ExportController/getOtherReports") ?>",
				dataType: "json",
				async: false,
				cache: false,
	...
	function DownloadExcel() {
		...
		if(company_name == "" || fromDate== "" || toDate==""){
			...
			location.href = "<?= base_url() ?>"+"ExportController/DownLoadExcelCashFund?comp="+btoa(company_name)+"&fromDate="+btoa(fromDate)+"&toDate="+btoa(toDate)+"&reportName="+btoa(reportName);
		}
	}
